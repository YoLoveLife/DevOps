...
def base_personplaybook():
    ...
    pb=PersonBook("install base soft","tomcat-server",'no')
    ...
    task5=PersonTask(module="yum",args="name=libaio state=present",)
    task6=PersonTask(module="yum",args="name=libselinux-python state=present",)
    task7=PersonTask(module="yum",args="name=expect state=present")
    pb.add_task(task1)
    pb.add_task(task2)
    pb.add_task(task3)
    pb.add_task(task4)
