from django.conf.urls import url
from .. import api
urlpatterns=[
    # Resource group api
    url(r'^v1/group/$', api.ManagerGroupListAPI.as_view()),
    url(r'^v1/group/create/$', api.ManagerGroupCreateAPI.as_view()),
    url(r'^v1/group/(?P<pk>[0-9]+)/update/$',api.ManagerGroupUpdateAPI.as_view()),
    url(r'^v1/group/(?P<pk>[0-9]+)/delete/$',api.ManagerGroupRemoveAPI.as_view()),

    # Resource host api
    url(r'^v1/hostbygroup/(?P<pk>[0-9]+)/$',api.ManagerHostListByGroupAPI.as_view()),
    url(r'^v1/host/(?P<pk>[0-9]+)/remove/', api.ManagerHostRemoveAPI.as_view()),
    url(r'^v1/host/(?P<pk>[0-9]+)/passwd/', api.ManagerHostPasswordAPI.as_view()),

    # Resource storage api
    url(r'^v1/storage/$', api.ManagerStorageListAPI.as_view()),
    url(r'^v1/storage/(?P<pk>[0-9]+)/remove/', api.ManagerStorageRemoveAPI.as_view()),

    # Resource search api
    url(r'^v1/search/',api.ManagerSearchAPI.as_view()),
]